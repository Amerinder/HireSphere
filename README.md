...
HireSphere is a full-stack AI-powered job portal SaaS platform with a Laravel 12 API backend, MongoDB persistence, React/Vite frontend, passwordless authentication, Redis queues, Resend email, and OpenAI-powered resume intelligence.
...
- Passwordless magic-link auth implemented with expiring single-use tokens
...
- Frontend auth context, protected routes, verification page, dashboard shell, and logout added
...
- `laravel/sanctum`: API token authentication for the magic-link login flow in Phase 2.
- `predis/predis`: Redis client for queue workers without requiring the PhpRedis extension.
- `resend/resend-php`: Resend email SDK for magic-link and notification emails.
...
- Collections planned: `users`, `companies`, `jobs`, `applications`, `saved_jobs`, `notifications`, `magic_links`, `analytics`, `resumes`
...
```http
POST http://localhost:8000/api/v1/auth/magic-link/verify
Accept: application/json
Content-Type: application/json

{
  "email": "student@gmail.com",
  "token": "64-character-token-from-the-magic-link"
}
```
...
- Verify your Resend sender/domain before expecting real magic-link emails.
...
- End-to-end smoke test passes for magic-link auth, job posting, applications, saved jobs, resume analysis fallback, candidate ranking, admin analytics, and security headers
...
- Resend email delivery requires `RESEND_API_KEY` and a verified sender/domain
